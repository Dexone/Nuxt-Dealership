...
const _sfc_main = {
  ...
  async setup(__props) {
    ...
    [__temp, __restore] = withAsyncContext(() => update()), await __temp, __restore();
    productStore.findSame();
    favouriteStore.findFavourite();
    return (_ctx, _push, _parent, _attrs) => {
      const _component_NuxtLink = __nuxt_component_0;
      _push(`<main${ssrRenderAttrs(mergeProps({ class: "bg-white max-w-7xl mx-auto" }, _attrs))}><div class="max-w-7xl mx-auto mb-3"></div><!--[-->`);
      ssrRenderList(unref(mainInfo), (main, index) => {
        ...
        if (unref(productStore).simile[main.id] > 0) {
          _push(ssrRenderComponent(_component_NuxtLink, { to: "/cart" }, {
            default: withCtx((_, _push2, _parent2, _scopeId) => {
              if (_push2) {
                _push2(`<button class="inline-flex items-center px-6 h-10 text-sm font-medium text-center text-gray-900 border border-gray rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300"${_scopeId}> \u0412 \u043A\u043E\u0440\u0437\u0438\u043D\u0435 &gt; </button>`);
              } else {
                return [
                  createVNode("button", { class: "inline-flex items-center px-6 h-10 text-sm font-medium text-center text-gray-900 border border-gray rounded-lg hover:bg-gray-100 focus:ring-4 focus:outline-none focus:ring-blue-300" }, " \u0412 \u043A\u043E\u0440\u0437\u0438\u043D\u0435 > ")
                ];
              }
            }),
            _: 2
        ...
        if (unref(productStore).simile[main.id] == 0) {
          _push(`<button class="inline-flex items-center px-10 h-10 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300"> \u041A\u0443\u043F\u0438\u0442\u044C </button>`);
        } else {
          _push(`<!---->`);
        }
        if (unref(favouriteStore).simile[main.id] == 0) {
...
//# sourceMappingURL=index-CV_-9B1y.mjs.map
