<template>
  <NuxtLink :to="`/product/${car._id}`">
    ...
    <!-- если фото из б/у раздела с фоном -->
    <div v-if="car.image[1].slice(car.image[1].length - 11) === 'used/1.webp'"
      class="flex flex-col items-center h-44 mt-2 mb-2  mx-4">
      <img class="rounded-lg object-cover size-full" :src="car.image[1]" />
    </div>

    <!-- если фото из нового раздела без фона -->
    <div v-else class="flex flex-col items-center h-48">
      <img class="rounded-t-lg" :src="car.image[1]" />
    </div>
  ...
  <div class="mx-auto max-w-fit flex mt-5 mb-5">
    ...
    <div :key="quantityCart = Object.values(cartStore.localCart).reduce(function (
      ...
      ">
      <!-- кн-а купить + лоадер -->
      <button v-if="quantityCart <= 0" @click="addToCart(car)"
        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center">
        <a v-if="loaderCart === false">Купить</a>
        <span v-if="loaderCart === true">
          <div role="status">
            <svg aria-hidden="true" class="w-8 h-4 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101"
      ...
      <!-- кн-а +- -->
      <ul v-if="quantityCart > 0" class="flex items-center -space-x-px h-10 text-base">
        <li>
          <a class="flex items-center justify-center px-4 h-10 ms-0 leading-tight text-gray-500 bg-white border border-e-0 border-gray-300 rounded-s-lg hover:bg-gray-100 hover:text-gray-700 d"
        ...
        <li>
          <a
            class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 hover:bg-gray-100 hover:text-gray-700"><a
              v-if="loaderCart === false">{{ quantityCart }}</a>
            <span v-if="loaderCart === true">
              <div role="status">
                <svg aria-hidden="true" class="w-4 h-4 text-gray-200 animate-spin fill-blue-600" viewBox="0 0 100 101"
                  fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z"
                    fill="currentColor" />
        ...
        <li>
          <a class="flex items-center justify-center px-4 h-10 leading-tight text-gray-500 bg-white border border-gray-300 rounded-e-lg hover:bg-gray-100 hover:text-gray-700"
    ...
    <div :key="quantityFavourite = Object.values(favouriteStore.localFavourite).reduce(
      ...
      <button v-if="quantityFavourite <= 0" @click="addToFavourite(car)"
        ...
        <a v-if="loaderFavourite === false"><svg v-if="loaderFavourite === false" class="w-6 h-6 text-gray-800"
      ...
      <button v-if="quantityFavourite > 0" @click="removeFavourite(car)"
        class="py-2 px-5 ms-3 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
