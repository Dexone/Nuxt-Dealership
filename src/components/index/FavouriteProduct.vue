<template>
  <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow inline-block m-5">
    <div class="flex items-center justify-between ml-4 mt-4 pr-2">
      ...
      <button type="button" @click="removeFavourite(favourite)"
    ...
    <NuxtLink :to="`/product/${favourite._id}`">
      ...
      <div v-if="favourite.image[0][1].slice(favourite.image[0][1].length - 11) === 'used/1.webp'"
        class="flex flex-col items-center h-44 mt-2 mb-2  mx-4">
        <img class="rounded-lg object-cover size-full" :src="favourite.image[0][1]" />
      </div>
    ...
    <div class="mx-auto max-w-fit" :key="quantityCart = Object.values(cartStore.localCart).reduce(function (
      accumulator,
      item
    ) {
      ...
      <button v-if="quantityCart === 0" @click="addToCart(favourite)"
