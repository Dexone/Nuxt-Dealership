<template>
      ...
      <div
        ...
        v-if="(Object.keys(favouriteStore.favourite)).includes(String(product.id)) === true"
      >
        <h5 class="mb-1 text-xl font-medium text-gray-900 ml-4 pt-4">
          <NuxtLink :to="`/product/${product.id}`"> {{ product.brand }} {{ product.model }} </NuxtLink>
        </h5>

        <p class="text-sm text-gray-500 ml-4 mr-4">
          {{ product.year }}г. / {{ product.power }}л.с. / {{ product.engine }} / {{ product.transmission }} /
          {{ product.kuzov }} / {{ product.color }}
        </p>

        <div class="flex flex-col items-center">
          <img class="rounded-t-lg" v-bind:src="product.image[1]" />
        </div>

        <h5 class="mb-1 text-xl font-medium text-gray-900 ml-4">{{ product.price }} ₽</h5>
        <span class="text-sm text-gray-500 ml-4"> {{ Math.round(product.price / 84) }} ₽/мес</span>

        <div class="flex flex-col items-center pb-10">
          <div class="flex mt-4 md:mt-6">
            <button
              v-if="(Object.keys(productStore.cart)).includes(String(product.id)) === false"
            ...
            <NuxtLink v-if="(Object.keys(productStore.cart)).includes(String(product.id)) === true" to="/cart"
              ><button
            ...
            >
              <svg v-if="loaderFavourite === false"
