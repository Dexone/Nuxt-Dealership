<template>
  <li>
    ...
    >
      <div class="block">
        <!-- если фото из б/у раздела -->
        <a
          class="inline-block -ml-3 mr-2"
          v-if="
            cart.image[0][1].slice(cart.image[0][1].length - 11) ===
            'used/1.webp'
          "
        >
          <img class="w-18 h-10 rounded-lg" :src="cart.image[0][1]" />
        </a>
        <!-- если фото из б/у раздела -->
        <a class="inline-block -ml-6" v-else>
      ...
      <div>
        ...
        <div class="items-center text-right mb-2">
          <button
            @click="cartStore.removeCart(cart)"
            class="ms-auto text-gray-500"
          >
            <!-- x -->
            <svg
              class="w-3 h-4 mr-1.5 pb-0.5"
              aria-hidden="true"
          ...
          <button
            :key="
              quantityFavourite = Object.values(
          ...
          >
            ...
            <svg
              v-if="loaderFavourite === false && quantityFavourite <= 0"
              @click="addToFavourite(cart)"
            ...
            <span v-if="loaderFavourite === true">
              <div role="status">
                <svg
                  aria-hidden="true"
                  class="w-5 h-4 text-gray-200 animate-spin fill-blue-600"
                  viewBox="0 0 100 101"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
        ...
        >
          <button
            @click="editCartMinus(cart)"
            type="button"
            class="inline-flex items-center px-2.5 py-1 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-lg hover:bg-gray-100"
          >
            -
          </button>
          ...
          <button
            @click="editCartPlus(cart)"
            type="button"
            class="px-2.5 py-1 text-sm font-medium text-gray-500 bg-white border border-gray-200 rounded-lg hover:bg-gray-100"
          >
            +
          </button>
        </div>
