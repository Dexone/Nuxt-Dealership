    ...
    <div class="max-w-sm bg-white border border-gray-200 rounded-lg shadow inline-block m-5"
      ...
      <div class="flex flex-col items-center pb-10">
        <div class="flex mt-4 md:mt-6">
          ...
          <NuxtLink v-if="(Object.keys(productStore.cart)).includes(String(main.id)) === true"><button
          ...
          <button v-if="(Object.keys(productStore.cart)).includes(String(main.id)) === false"
          ...
          <!-- v-if="возврат true, если эта карточка есть в избранном" -->
                    <button @click="deleteFromFavourite(main.id)" v-if="(Object.keys(favouriteStore.favourite)).includes(String(main.id)) === true"
          ...
          <button v-if="(Object.keys(favouriteStore.favourite)).includes(String(main.id)) === false"
            @click="addToFavourite(main)"
            class="py-2 px-4 ms-2 text-sm font-medium text-gray-900 focus:outline-none bg-white rounded-lg border border-gray-200 hover:bg-gray-100 hover:text-blue-700 focus:z-10 focus:ring-4 focus:ring-gray-100">
            <svg class="w-6 h-6 text-gray-800" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="24"
