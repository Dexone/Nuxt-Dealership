    ...
    <template v-if="mainInfo?.length > 0">
      ...
      >
        <CardProduct
          :car="main"
          v-if="
            main.price >= paramsStore.selectedParams.price[0] &&
            main.price <= paramsStore.selectedParams.price[1] &&
            main.power >= paramsStore.selectedParams.power[0] &&
            main.power <= paramsStore.selectedParams.power[1]
          "
        />
      </div>
...
async function getProducts() {
  ...
  const { data } = await useApiCarData(
    `api/products?${route.params.searchString}`
  )
  console.log(data)
  try {
