...
export const useFavouriteUser = defineStore('favouriteUser', {
  ...
  actions: {
    async addToFavourite(value) {
      const getData = await $fetch(`https://martynovd.ru/shop-api/favourite/${useProduct().user}`, {
        method: 'GET'
      }); //получить корзину пользователя
      let editData = getData.favourites;
      editData[value.id] = 1;
      const { data } = await $fetch(`https://martynovd.ru/shop-api/favourite/${useProduct().user}`, {
        method: 'PATCH',
    ...
    findFavourite() {
      ...
      setTimeout(() => {
        axios.get(`https://martynovd.ru/shop-api/favourite/${useProduct().user}`).then((res) => {
          useFavourite().simile = {};
